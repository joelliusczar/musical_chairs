...
def test_post_with_only_name(
	...
	nextId = data["id"]
	assert nextId == dtos.encode_station_id(len(initialStations) + 1)
...
def test_request_song(
	...
	response = client.post(
		f"stations/testUser_bravo/romeo_station/request/0/{dtos.encode_song_id(36)}",
		headers=headers
	)
	assert response.status_code == 200
...
def test_request_song_only_one_staion_allowed(
	...
	response = client.post(
		f"stations/testUser_bravo/papa_station/request/0/{dtos.encode_song_id(40)}",
		headers=headers
	)
	assert response.status_code == 403
...
def test_request_song_any_station(
	...
	response = client.post(
		f"stations/2/romeo_station/request/0/{dtos.encode_song_id(36)}",
		headers=headers
	)
	assert response.status_code == 200

	response = client.post(
		f"stations/2/papa_station/request/0/{dtos.encode_song_id(40)}",
		headers=headers
	)
	assert response.status_code == 200

	response = client.post(
		f"stations/2/oscar_station/request/0/{dtos.encode_song_id(30)}",
		headers=headers
	)
	assert response.status_code == 200

	response = client.post(
		f"stations/2/sierra_station/request/0/{dtos.encode_song_id(15)}",
		headers=headers
	)
	assert response.status_code == 200
...
def test_request_song_timeout(
	...
	#1 t=0 [2]
	response = client.post(
		f"stations/testUser_bravo/{dtos.encode_station_id(3)}/request/0/{dtos.encode_song_id(36)}",
		headers=headers
	)
	assert response.status_code == 200
	#2 t=s+15 [3]
...
def test_get_station_user_rule_selection(
	...
	response = client.get(
		f"stations/2/{dtos.encode_station_id(4)}/catalogue",
		headers=headers
	)
	...
	response = client.get(
		f"stations/testUser_bravo/{dtos.encode_station_id(3)}/catalogue",
		headers=headers
	)
	...
	response = client.get(
		f"stations/2/{dtos.encode_station_id(3)}/catalogue",
		headers=headers
	)
...
def test_get_station_for_edit_with_view_security(
	...
	data = json.loads(response.content)
	assert response.status_code == 200
	assert data["id"] == dtos.encode_station_id(3)
	...
	assert data["id"] == dtos.encode_station_id(13)
	...
	data = json.loads(response.content)
	assert response.status_code == 200
	assert data["id"] == dtos.encode_station_id(12)
	...
	data = json.loads(response.content)
	assert response.status_code == 200
	assert data["id"] == dtos.encode_station_id(14)
	...
	data = json.loads(response.content)
	assert response.status_code == 200
	assert data["id"] == dtos.encode_station_id(15)
...
def test_rule_adding_validation(
	...
	response = client.post(
		f"stations/2/{dtos.encode_station_id(1)}/user_role?subjectuserkey={28}",
		headers=headers,
		json=rule
	)
	assert response.status_code == 403
